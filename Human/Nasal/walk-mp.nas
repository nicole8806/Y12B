...
#	}
	if (falling) {	# 13,000 to 12,000 ft = 10 sec. 12,000 - 4,000 = 44 sec.
		...
		if (posz_geo < posz1) {	# ground is below walker
			#zhangfp add
			#posz_geo is altitude of ground and posz1 is altitude of walker
			setprop("engines/engine[3]/fuel-px-psi", posz_geo);
			#zhangfp end
			print(sprintf("posz_geo = %5.8f",posz_geo));
	...
	#操作操纵棒使人的方向改变，以调整着陆位置，此处需要改进
	var walker_psi = getprop("sim/current-view/heading-offset-deg");
	var pull_strength = getprop("engines/engine/oil-pressure-psi");
	print(sprintf("pull_strength %d",pull_strength));
	
	if(pull_strength != nil){
		var degree_roll = pull_strength/MAX_STRENTH*12.0/25;
		walker_psi = walker_psi - degree_roll; #degree_roll/36,左拉角度增加，右拉角度减小
...
#		#model_turn = model_turn ;
		...
		var turn_add = pull_strength/36000*1000;
		if (turn_add == nil) {turn_add = 0;}
		if (pull_strength != 0)
		{
			model_turn = model_turn + pull_strength/36000*1000;
			print(sprintf("model_turn1 %d",model_turn));
		}
		
#		print(sprintf("model_turn + pull_strength :%d",model_turn ));
		if (model_turn != nil)
		{
			setprop("sim/walker/model-heading-deg",model_turn);
			print(sprintf("model_turn2 %d",model_turn));
		}
	}
	#=============================xuy==============================================
	...
	var mm = 100000 / ERAD * DEG2RAD;   #每移动1毫米，相当于移动了多少度
	...
	var posy2_t = speed_level*cos(degree_proc)*mm/100000;	#除回去100000
	var posx2_t = speed_level*sin(degree_proc)*mm/100000;
...
var init_common = func {
	...
	setlistener("engines/engine[3]/oil-pressure-psi", func(n) {
		if ((n.getValue() == 10))
		{
			...
			{
				print(sprintf("jump_status == true"));
				#land is false
				setprop("engines/engine[0]/mp-osi",0);
				#chute is not open
				setprop("engines/engine[3]/mp-osi",0);
				setprop("sim/walker/key-triggers/outside-toggle","false");
			}
